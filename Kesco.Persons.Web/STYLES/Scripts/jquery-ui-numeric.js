...
(function($)
{
    ...
    $.widget('ui.numeric', {
        ...
        _create: function()
        {
			...
			w.attr("input-culture", o.format.culture?o.format.culture:'');
            ...
            w.attr('title', o.title).wrap($('<div class="ui-widget ui-widget-header ui-corner-all ui-numeric" />'));
        ...
        _setOption: function(key, value)
        {
            ...
            {
				case 'culture':
					if (o.format.culture != value) {
						o.format.culture = value;
						w.attr("input-culture", value?value:'');
						t._setInputValue(t._value);
					}
					break;
                ...
                case 'minValue':
                    o.minValue = value === false ? false : _numVal2(value, o.format.culture);
                    if (o.minValue !== false && t._value < o.minValue)
                        t._setValue(o.minValue);
                    break;

                case 'maxValue':
                    o.maxValue = value === false ? false : _numVal2(value, o.format.culture);
                    if (o.maxValue !== false && t._value > o.maxValue)
        ...
        _getInputValue: function(val, parse)
        {
            ...
            return parse ? _numVal2(val, this.options.format.culture) : val;
		...
		var format = this.options.showCurrency ? ("C"+this.options.format.precision): ("N"+this.options.format.precision)
            this.widget().attr('value', $.global.format(val, format, this.options.format.culture)).change();
        ...
        {
            var t = this;
            val = _numVal2(val, t.options.format.culture);
        ...
        _format: function(val)
        {
			...
			var format = this.options.showCurrency ? ("C"+this.options.format.precision): ("N"+this.options.format.precision)
            return isNaN(val) || (o.emptyValue !== false && val === o.emptyValue) ? '' : $.format(val, format, this.options.format.culture);
        },
        ...
        _onKeyUp: function()
        {
            ...
            var t = this, v = t._getInputValue(t.widget().attr('value'), true);
            if (!isNaN(v))
                t._value = v;
    ...
    function _numVal(val)
    {
        if (typeof val !== 'number')
            val = $.global.parseFloat(val, 10, this.options.format.culture);
    ...
    function _numVal2(val, culture)
    {
        if (typeof val !== 'number')
            val = $.global.parseFloat(val, 10, culture);
